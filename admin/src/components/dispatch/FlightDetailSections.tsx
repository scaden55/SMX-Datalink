  ...
  return (
    <div className="border-b border-[var(--surface-3)]">
      ...
      >
        ...
        <span className="ml-auto text-[12px] tabular-nums text-[var(--text-primary)] truncate max-w-[400px]">
          {summary}
        </span>
        <svg
...
const inputCls =
  'bg-[var(--surface-2)] border border-[var(--surface-3)] text-[12px] tabular-nums text-[var(--text-primary)] rounded-md px-1.5 py-0.5 outline-none focus:border-blue-400';
const readOnlyCls =
  'text-[12px] tabular-nums text-[var(--text-primary)]';
const labelCls =
  'text-[9px] font-medium uppercase tracking-[0.06em] text-[var(--text-muted)]/70';
...
function FuelTableRow({
  ...
  return (
    <div className={`flex items-center py-1 ${indent ? 'pl-4' : ''} border-b border-[var(--surface-3)]/20 last:border-b-0 ${highlighted ? 'border-l-2 border-l-amber-400 bg-amber-400/5 pl-1' : ''}`}>
      <span className={`text-[12px] w-[130px] shrink-0 ${bold ? 'font-semibold text-[var(--text-primary)]/80' : 'text-[var(--text-muted)]'}`}>
        {label}
      </span>
      <span className="text-[12px] tabular-nums text-[var(--text-muted)] w-[55px] shrink-0 text-right pr-2">
        {time || ''}
      </span>
      {editable && fieldKey && onFieldChange ? (
        ...
        <span className={`text-[12px] tabular-nums w-[80px] shrink-0 text-right ${warn ? 'text-amber-400' : 'text-[var(--text-primary)]'}`}>
          {value}
        </span>
      )}
      {note && (
        <span className="text-[11px] text-[var(--text-muted)] ml-3 truncate">{note}</span>
      )}
    </div>
  );
}
...
function WeightTableRow({
  ...
  return (
    <div className={`flex items-center py-1 border-b border-[var(--surface-3)]/20 last:border-b-0 ${highlighted ? 'border-l-2 border-l-amber-400 bg-amber-400/5 pl-1' : ''}`}>
      <span className="text-[12px] text-[var(--text-muted)] w-[90px] shrink-0">{label}</span>
      {editable && fieldKey && onFieldChange ? (
        <input
          type="text"
        ...
        <span className="text-[12px] tabular-nums text-[var(--text-primary)] w-[100px] shrink-0 text-right">
          {estimated}
        </span>
      )}
      {maximum !== undefined && (
        <span className="text-[12px] tabular-nums text-[var(--text-muted)] w-[100px] shrink-0 text-right">
          {maximum}
        </span>
      )}
      {margin !== undefined && (
...
export default function FlightDetailSections({ flight }: { flight: DispatchFlight }) {
  ...
  const formData = flight.flightPlanData;
  const w = ofp?.weights;
  const f = ofp?.fuel;
  const t = ofp?.times;

  // -- Aircraft summary
  const tail = flight.bid.aircraftRegistration ?? '---';
  ...
  const melText = ((editableFields.melRestrictions as string) ?? formData?.melRestrictions ?? '').trim();
  ...
  // -- Route summary
  const route = (editableFields.route as string) ?? formData?.route ?? ofp?.route ?? '';
  ...
  const altIcao = (editableFields.alternate1 as string) ?? formData?.alternate1 ?? '';
  ...
  return (
    <div>
      {/* -- Aircraft -------------------------------------------------------- */}
      <DetailRow title="Aircraft" summary={acftSummary} hasData={!!acType && acType !== '---'}>
        <FieldGrid>
          <ReadOnlyField label="Tail Number" value={tail} />
          <ReadOnlyField label="Aircraft Type" value={acType} />
          <ReadOnlyField label="Title" value={ofp?.aircraftType ?? acType} />
          <EditableField
            label="Cruise FL"
            value={(editableFields.cruiseFL as string) ?? formData?.cruiseFL ?? ''}
            fieldKey="cruiseFL"
            canEdit={canEdit}
          ...
          <EditableField
            label="AOB FL"
            value={(editableFields.aobFL as string) ?? formData?.aobFL ?? ''}
            fieldKey="aobFL"
            canEdit={canEdit}
            onFieldChange={onFieldChange}
            placeholder="FL350"
          ...
          <EditableField
            ...
            value={(editableFields.pic as string) ?? formData?.pic ?? flight.pilot.name ?? ''}
            fieldKey="pic"
            canEdit={canEdit}
            onFieldChange={onFieldChange}
      ...
      <DetailRow title="MEL & Restrictions" summary={melSummary} hasData amber={hasMEL}>
        {canEditMEL ? (
          <div className={hl('melRestrictions') ? 'border-l-2 border-amber-400 bg-amber-400/5 pl-1.5' : ''}>
            ...
            <textarea
              value={(editableFields.melRestrictions as string) ?? formData?.melRestrictions ?? ''}
              onChange={(e) => onFieldChange('melRestrictions', e.target.value)}
              className="w-full h-20 rounded bg-[var(--surface-0)] border border-[var(--surface-3)] text-[var(--text-primary)] text-[12px] px-2 py-1.5 tabular-nums resize-none focus:outline-none focus:border-sky-400"
          ...
          <div className="space-y-1.5 text-[12px]">
            {hasMEL ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[var(--text-muted)]">No active MELs or restrictions</span>
              </div>
            )}
          </div>
        )}
      ...
      <DetailRow title="Route" summary={routeSummary} hasData={!!route}>
        <div className="space-y-2">
          <div className="flex items-center gap-3 text-[12px]">
            <ReadOnlyField label="Origin" value={flight.bid.depIcao ?? '---'} />
            <span className="text-[var(--text-muted)] mt-3">&rarr;</span>
            <ReadOnlyField label="Destination" value={flight.bid.arrIcao ?? '---'} />
            <ReadOnlyField label="Distance" value={
              flight.bid.distanceNm
                ? `${fmt(flight.bid.distanceNm)} nm`
                : '---'
            } />
            <ReadOnlyField label="Est Enroute" value={fmtTime(t?.estEnroute ?? flight.bid.flightTimeMin)} />
          ...
          {canEditRoute ? (
            ...
            <div className={hl('route') ? 'border-l-2 border-amber-400 bg-amber-400/5 pl-1.5' : ''}>
              ...
              <div className="tabular-nums text-[12px] text-[var(--text-primary)] leading-relaxed break-all">
                {route || <span className="text-[var(--text-muted)] italic">No route loaded</span>}
              </div>
            </div>
          )}
          <div className="flex items-center gap-4 text-[12px]">
            <EditableField
              label="Alternate 1"
              value={(editableFields.alternate1 as string) ?? formData?.alternate1 ?? ''}
              fieldKey="alternate1"
              canEdit={canEdit}
              onFieldChange={onFieldChange}
              placeholder="ICAO"
            ...
            <EditableField
              label="Alternate 2"
              value={(editableFields.alternate2 as string) ?? formData?.alternate2 ?? ''}
              fieldKey="alternate2"
              canEdit={canEdit}
              onFieldChange={onFieldChange}
              placeholder="ICAO"
      ...
      <DetailRow title="Fuel" summary={fuelSummary} hasData={!!f}>
        <div>
          {/* Column headers */}
          <div className="flex items-center pb-1 mb-1 border-b border-[var(--surface-3)]/40">
          ...
          <FuelTableRow
            ...
            note={`${flight.bid.depIcao ?? '---'}-${flight.bid.arrIcao ?? '---'}`}
            bold
          />
          <FuelTableRow
            label="Burn"
          ...
          {(ofp?.alternates ?? []).length > 0 && (
            <>
              <div className="flex items-center py-1.5 mt-1">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-[var(--text-muted)]/70">Alternates</span>
              </div>
        ...
        <div className="space-y-2">
          ...
          {ofp?.steps && ofp.steps.length > 0 && (
            <div>
              ...
              <div className="max-h-[140px] overflow-y-auto rounded border border-[var(--surface-3)]/30">
                <div className="flex items-center px-2 py-1 border-b border-[var(--surface-3)]/40 bg-[var(--surface-2)]/30">
                ...
                {ofp.steps
                  ...
                  .map((step, i) => (
                    <div
                      key={`${step.ident}-${i}`}
                      className="flex items-center px-2 py-0.5 border-b border-[var(--surface-3)]/10 last:border-b-0"
                    >
                      <span className={`text-[11px] tabular-nums w-[70px] shrink-0 ${
                        step.fixType === 'toc' || step.fixType === 'tod' ? 'text-blue-400 font-semibold' : 'text-[var(--text-primary)]'
                      }`}>
                        {step.fixType === 'toc' ? 'T/C' : step.fixType === 'tod' ? 'T/D' : step.ident}
                      </span>
                      <span className="text-[11px] tabular-nums text-[var(--text-primary)] w-[60px] shrink-0 text-right">
                        {fmt(step.altitudeFt)}
                      </span>
                      <span className="text-[11px] tabular-nums text-[var(--text-muted)] w-[60px] shrink-0 text-right">
                        {fmt(step.distanceFromOriginNm)}
                      </span>
                      <span className="text-[11px] tabular-nums text-[var(--text-muted)] w-[80px] shrink-0 text-right">
                        {fmt(step.fuelRemainLbs)}
                      </span>
                      <span className="text-[11px] tabular-nums text-[var(--text-muted)] w-[50px] shrink-0 text-right">
                        {step.wind || '---'}
                      </span>
                      <span className="text-[11px] tabular-nums text-[var(--text-muted)] w-[40px] shrink-0 text-right">
