...
import { Plus, Pencil, Trash2, MoreVertical, ChevronLeft, ChevronRight, RotateCcw } from 'lucide-react';
...
function OverhaulProgress({ hours, interval }: { hours: number; interval: number }) {
  const pct = Math.min(hours / interval, 1);
  let barColor = 'var(--accent-emerald)';
  if (pct > 0.95) barColor = 'var(--accent-red)';
  else if (pct > 0.8) barColor = 'var(--accent-amber)';
...
export function ComponentsTab({ aircraftId }: { aircraftId: number }) {
  ...
  const [overhaulItem, setOverhaulItem] = useState<AircraftComponent | null>(null);
  ...
  async function handleOverhaulReset() {
    if (!overhaulItem) return;
    try {
      await api.post(`/api/admin/maintenance/components/${overhaulItem.id}/overhaul`, {});
      toast.success(`Overhaul reset for ${overhaulItem.partNumber}`);
      setOverhaulItem(null);
      fetchItems();
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : 'Failed to reset overhaul');
    }
  }
  ...
  return (
    <>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <th className={COL_HEADER_CLASS} style={{ ...colHeaderStyle, textAlign: 'right' }}>Hrs Since Ovhl</th>
              ...
              <motion.tbody variants={tableContainer} initial="hidden" animate="visible">
                {paged.map((item) => {
                  const showProgress =
                    item.overhaulIntervalHours != null &&
                    item.overhaulIntervalHours > 0 &&
                    item.hoursSinceOverhaul != null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`${CELL_CLASS} ${MONO_CLASS}`} style={{ ...cellStyle, textAlign: 'right' }}>
                        {item.hoursSinceNew != null ? formatHours(item.hoursSinceNew) : '\u2014'}
                      </td>
                      ...
                      <td className={`${CELL_CLASS} ${MONO_CLASS}`} style={{ ...cellStyle, textAlign: 'right' }}>
                        <div>
                          {item.hoursSinceOverhaul != null ? formatHours(item.hoursSinceOverhaul) : '\u2014'}
                          {showProgress && (
                            <OverhaulProgress
                              hours={item.hoursSinceOverhaul!}
                              interval={item.overhaulIntervalHours!}
                            />
                          )}
                        </div>
                      </td>
                      ...
                      <td className={CELL_CLASS} style={{ ...cellStyle, padding: '6px 8px' }}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => setOverhaulItem(item)}>
      ...
      {/* ── Overhaul Reset Confirmation ─────────────────────────── */}
      <Dialog open={!!overhaulItem} onOpenChange={(o: boolean) => { if (!o) setOverhaulItem(null); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Reset Overhaul</DialogTitle>
            <DialogDescription>
              Reset overhaul tracking for component{' '}
              <span className={MONO_CLASS} style={{ fontWeight: 600 }}>
                {overhaulItem?.partNumber}
              </span>
              {overhaulItem?.serialNumber ? ` (S/N ${overhaulItem.serialNumber})` : ''}?
              This will zero out hours and cycles since overhaul.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOverhaulItem(null)}>
              Cancel
            </Button>
            <Button onClick={handleOverhaulReset}>
              Reset
            </Button>
          </DialogFooter>
        </DialogContent>
...
// ═══════════════════════════════════════════════════════════════
// ComponentFormDialog — create/edit form (per-aircraft, no fleet dropdown)
// ═══════════════════════════════════════════════════════════════
...
function ComponentFormDialog({
  ...
  async function handleSave() {
    if (!partNumber.trim() || !serialNumber.trim()) return;
    setSaving(true);
    try {
      const payload = {
        aircraftId,
  ...
  const canSave = partNumber.trim() && serialNumber.trim();
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ...
              : 'Add a new life-limited component to this aircraft.'}
          </DialogDescription>
        </DialogHeader>
