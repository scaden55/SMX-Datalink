    ...
    <div className="border-b border-acars-border px-4 py-2.5">
          ...
          {depTimeLabel && (
            <>
              ...
              <span className={depTimeLabel === 'ATD' ? 'text-emerald-400/80' : 'text-acars-muted/70'}>
                {depTimeLabel} <span className="text-acars-text/80 tabular-nums">{depTimeValue}</span>
              </span>
            </>
          )}
        </div>
        <span className="text-center shrink-0 px-3 text-[11px] font-medium text-blue-400/80 tabular-nums uppercase">
          {pilotCallsign || '---'}
        </span>
          ...
          {arrTimeLabel && (
            <>
              ...
              <span className={arrTimeLabel === 'ATA' ? 'text-emerald-400/80' : 'text-acars-muted/70'}>
                {arrTimeLabel} <span className="text-acars-text/80 tabular-nums">{arrTimeValue}</span>
              </span>
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center mt-1">
        <div>
          ...
          {punctuality === 'delayed' && (
            <span className="inline-flex items-center px-1.5 py-0 rounded-[2px] text-[10px] font-semibold uppercase tracking-[0.06em] bg-amber-500/15 text-amber-400">
