  ...
  return (
    <div className="w-[260px] shrink-0 border-l border-white/[0.06] flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/[0.06]">
        <span className="text-[11px] uppercase tracking-[0.08em] text-acars-muted font-medium">My Bids</span>
        <span className="inline-flex items-center justify-center min-w-[18px] h-4 rounded-[2px] bg-acars-badge-bg text-[10px] font-semibold text-acars-badge-text tabular-nums px-1">
          {bids.length}
        </span>
      </div>
      <div className="flex-1 overflow-auto">
        {bids.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <p className="text-[10px] text-acars-muted/50 mt-0.5">Place a bid from the cargo schedule</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="tabular-nums font-semibold text-[12px] text-acars-text">{bid.flightNumber}</span>
                    {isActive && (
                  ...
                  <div className="flex items-center gap-1.5 text-[12px] mb-1">
                    ...
                    <ArrowRight className="w-2.5 h-2.5 text-[#6b8aff]/40" />
                  ...
                  <div className="flex items-center gap-2 text-[11px] text-acars-muted">
                    ...
                    <span className="flex items-center gap-0.5 tabular-nums text-acars-muted">
                      <Clock className="w-2.5 h-2.5" />
                      {bid.depTime}Z
                    </span>
                    <span className="tabular-nums text-acars-muted">{formatDuration(bid.flightTimeMin)}</span>
                  </div>
                </button>
              );
            })}
      ...
      <div className="px-3 py-2 border-t border-white/[0.06] flex items-center justify-center">
        <button
          ...
          disabled={!ofp}
          className="flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-400/20 hover:bg-emerald-500/20 transition-colors duration-150 disabled:opacity-30 disabled:cursor-not-allowed"
