...
function FlightCatBadge({ cat }: { cat: string | null }) {
  ...
  return (
    <span className={`inline-flex px-[3px] rounded-[2px] text-[12px] font-bold uppercase tracking-[0.08em] ${colors[cat] ?? 'text-acars-muted bg-acars-input'}`}>
      {cat}
    </span>
  );
}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2">
        <span className="text-[12px] uppercase tracking-[0.08em] text-acars-muted font-medium">{label}</span>
      ...
      {wx?.metar ? (
        ...
        <p className="text-[12px] text-acars-muted">No METAR available</p>
      )}
      {wx?.taf ? (
        <pre className="text-[12px] tabular-nums text-acars-muted bg-acars-input rounded-md px-2 py-1 whitespace-pre-wrap break-words border border-acars-border">
          {wx.taf.rawTaf}
        </pre>
      ) : (
        <p className="text-[12px] text-acars-muted">No TAF available</p>
      )}
    </div>
  );
}
...
export function PlanningWeatherTab() {
  ...
  if (icaos.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center gap-2 p-4">
        ...
        <p className="text-[12px] text-acars-muted">Enter origin/destination to fetch weather</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-3 space-y-3 overflow-auto">
      {weatherLoading && <p className="text-[12px] text-blue-400 animate-pulse">Fetching weather...</p>}
      {icaos.map((e) => (
