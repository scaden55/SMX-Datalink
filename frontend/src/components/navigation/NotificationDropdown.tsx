  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 flex items-center justify-center min-w-[14px] h-3.5 rounded-full bg-acars-red text-[8px] font-bold text-white px-0.5">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-[9999] w-80 max-h-96 flex flex-col rounded-md border border-acars-border bg-[#1c2433] shadow-xl">
          {/* Header */}
          <div className="flex items-center justify-between px-3 py-2 border-b border-acars-border">
            ...
            {unreadCount > 0 && (
              <button
                onClick={() => markAllRead()}
                className="flex items-center gap-1 text-[10px] text-acars-blue hover:text-acars-blue/80 transition-colors"
              >
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 border-b border-acars-border/50 hover:bg-[#161b22] transition-colors ${
                    !n.read ? 'bg-acars-blue/5' : ''
                  }`}
                >
                  <div className="flex items-start gap-2">
                    {!n.read && (
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-acars-blue shrink-0" />
                    )}
